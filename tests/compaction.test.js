...
adapters.forEach(function (adapter) {
  describe('suite2 test.compaction.js-' + adapter, function () {
    ...
    // we can drop this.
    if (
      autoCompactionAdapters.indexOf(adapter) === -1 ||
      ...
      return db
        ...
        .then(function () {
          return Promise.all(
            ...
            att1.should.equal(data2, 'att1 is the final one, not att2')
          })
        })
    })
    ...
    it('#2818 Compaction removes attachments given conflicts', function () {
      ...
      return db
        ...
        .then(function () {
          return Promise.all(
            ['1-a1', '2-a2', '3-a3', '1-b1'].map(function (rev) {
        ...
        .then(function (docs) {
          ...
          allDigests = allDigests
            .concat(digestsToForget)
            .concat(digestsToRemember)

          return Promise.all(
            allDigests.map(function (digest) {
              var doc = {
                _attachments: {
                  'newatt.txt': {
                    content_type: 'text/plain',
    ...
    it('#2818 Compaction really replaces attachments', function () {
      ...
      return db
        ...
        .then(function (doc1) {
          doc1._attachments['att.txt'].data.should.equal(
            att1,
        ...
        .then(function (doc2) {
          doc2._attachments['att.txt'].data.should.equal(
            att1,
        ...
        .then(function (doc3) {
          doc3._attachments['att.txt'].data.should.equal(
            att2,
        ...
        .then(function (doc4) {
          doc4._attachments['att.txt'].data.should.equal(
            att2,
            'md5-colliding content was really replaced'
          )
        })
        ...
        db.post(doc, function (err, res) {
          ...
          db.post(doc, function (err, res) {
            ...
            db.get('doc', { rev: rev1 }, function (err) {
              ...
              err.name.should.equal('not_found', 'rev-1 should be missing')
              db.get('doc', { rev: rev2 }, function (err) {
                err.status.should.equal(404, 'rev-2 should be missing')
                err.name.should.equal('not_found', 'rev-2 should be missing')
