  ...
  describe('test.changes.js-' + adapter, function () {
    ...
    it('All changes', function (done) {
      ...
      db.post({ test: 'somestuff' }, function () {
        var promise = db.changes({}).on('change', function (change) {
    ...
    it('Changes with filter not present in ddoc', async function () {
      ...
      await assert.isRejected(changes, 'missing', 'completes with an exception')
      ...
      const docs = [
        ...
        { _id: '1', integer: 1 },
        {
          ...
          views: {
            even: {
              map:
                'function (doc) { if (doc.integer % 2 === 1)' +
                ' { emit(doc._id, null) }; }'
            }
          }
        }
    ...
    it('3356 throw inside a filter', function (done) {
      ...
      db.put({
        ...
        .should.eventually.be.fulfilled.then(function () {
          return db.changes({ filter: 'test/test' }).should.eventually.be
            .rejected
        })
        .then(function () {
          done()
    ...
    it('Changes with missing param `view` in request', function (done) {
      ...
      testUtils.writeDocs(db, docs, function () {
        db.changes({
          ...
          err.status.should.equal(
            testUtils.errors.BAD_REQUEST.status,
            'correct error status returned'
          )
          ...
          db.bulkDocs({ docs: docs2 }, function () {
            db.changes({
              ...
              .on('complete', function (results) {
                ...
                findById(changes, '3').doc.integer.should.equal(3)

                findById(changes, '5').doc.integer.should.equal(5)
    ...
    it('Changes with style = all_docs and a callback for complete', function (done) {
      var simpleTree = [
        [
          { _id: 'foo', _rev: '1-a', value: 'foo a' },
          { _id: 'foo', _rev: '2-b', value: 'foo b' },
    ...
    it('Changes doc', function (done) {
      ...
      db.post({ test: 'somestuff' }, function () {
        db.changes({
          ...
          change.doc._rev.should.equal(
            change.changes[change.changes.length - 1].rev
          )
          done()
        })
      })
    ...
    it('#2569 Non-live doc_ids filter', function () {
      var docs = [{ _id: '0' }, { _id: '1' }, { _id: '2' }, { _id: '3' }]
      var db = new PouchDB(dbs.name)
      return db
        .bulkDocs(docs)
        .then(function () {
