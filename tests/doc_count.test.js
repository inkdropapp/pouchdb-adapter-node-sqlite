...
describe('test.doc_count.js-sqlite3', function () {
  ...
  function readMeta(raw) {
    return raw
      .prepare("SELECT doc_count, doc_count_seq FROM 'metadata-store'")
      .get()
  }
  ...
  it('recounts on open when the stored count is stale', async function () {
    ...
    withRawDb(function (raw) {
      raw.prepare("UPDATE 'metadata-store' SET doc_count = 100").run()
      raw
        .prepare(
  ...
  it('migrates a database created before the stored count', async function () {
    ...
    withRawDb(function (raw) {
      raw.exec("ALTER TABLE 'metadata-store' DROP COLUMN doc_count")
      raw.exec("ALTER TABLE 'metadata-store' DROP COLUMN doc_count_seq")
      raw.exec("UPDATE 'metadata-store' SET db_version = 7")
    })
    ...
    withRawDb(function (raw) {
      raw
        .prepare("SELECT db_version FROM 'metadata-store'")
        .get()
